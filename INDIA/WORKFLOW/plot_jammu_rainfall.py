...
SHAPEFILE_PATH = os.path.join(
    os.path.dirname(__file__),
     "/home/ras_08/WEATHER/INDIA/WORKFLOW/JK&LADAKH/LADAKH_DISTRICTS.geojson"
)
...
def load_shapefile() -> gpd.GeoDataFrame:
    """Load JK shapefile. Reprojects to EPSG:4326 if needed."""
    if not os.path.exists(SHAPEFILE_PATH):
        raise FileNotFoundError(
            f"Shapefile not found: {SHAPEFILE_PATH}\n"
            f"Please update SHAPEFILE_PATH in the script, or place\n"
            f"/home/ras_08/WEATHER/INDIA/WORKFLOW/JK&LADAKH/JAMMU & KASHMIR_DISTRICTS.geojson  in the same folder as this script."
        )
...
def apply_map_features(ax, gdf: gpd.GeoDataFrame, outer_mask):
    ...
    gdf.boundary.plot(ax=ax, edgecolor="#CC00CC", linewidth=0.6, zorder=6)
    outer_union = unary_union(gdf.geometry.values)   # now Ladakh + J&K together
    outer_gdf = gpd.GeoDataFrame({"geometry": [outer_union]}, crs=gdf.crs)
    outer_gdf.boundary.plot(ax=ax, edgecolor="#880000", linewidth=2.0, zorder=7)
...
def set_title(ax, param_label, forecast_hours, based_on_str, valid_for_str):
    ax.set_title(
        f"{MODEL_NAME} ({NWP_RES}) JAMMU&KASHMIR,LADAKH {param_label} ({forecast_hours}-HR FCST)\n"
...
def run(wrf_path: str, output_dir: str, no_animation: bool = False):
    ...
    daily_dir = os.path.join(output_dir, "J&K_LADAKH_rainfall")
    ...
    outer_mask = make_outer_mask(gdf)   # now correctly spans both regions
    ...
    add_weatherex_logo(fig)
    out_path = os.path.join(output_dir, "J&k_LADAKH_cumulative_rainfall.png")
    ...
    for day_num, s_idx, e_idx, s_ts, e_ts in day_intervals:
        ...
        add_weatherex_logo(fig)
        fname    = f"J&K_LADAKH_daily_Day{day_num}_{e_ts.strftime('%d%b%Y').upper()}.png"
    ...
    if no_animation:
        ...
        cf_anim  = None
        anim_out = os.path.join(output_dir, "J&K_LADAKH_hourly_animation.mp4")
...
def parse_args():
    p = argparse.ArgumentParser(
        description="JK,LADAKH WRF rainfall plots using "
    )
    p.add_argument(
        "--input", required=True,
        help="Path to a single wrfout file, e.g. wrfout_d01_2024-06-01_00:00:00  /home/ras_08/WEATHER/INDIA/WORKFLOW/JK&LADAKH/LADAKH_DISTRICTS.geojson"
    )
    p.add_argument(
        "--outdir", default="./mumbai_output",
        help="Directory for output images and animation (default: ./JK_LADAKH)"
    )
    p.add_argument(
        "--shapefile", default=None,
        help="Override path to  /home/ras_08/WEATHER/INDIA/WORKFLOW/JK&LADAKH/LADAKH_DISTRICTS.geojson"
    )
    p.add_argument(
