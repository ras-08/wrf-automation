...
def run_wind_vector_pipeline(wrf_path, output_dir):
    ...
    def format_ist_time(wrf_time_raw):
    ...
    vec = ax.quiver(to_np(lons[::skip, ::skip]), to_np(lats[::skip, ::skip]),
                    ...
                    transform=ccrs.PlateCarree(), color='black', scale=180, width=0.002, headwidth=3,headlength=4,headaxislength=3.5,  alpha=0.75, zorder=10)

    plt.colorbar(cf, ax=ax, orientation='horizontal', pad=0.03, shrink=0.8,
                 label='Max Wind Speed (m/s)')

    start_t = format_ist_time(uv_met.Time.values[0])
    end_t = format_ist_time(uv_met.Time.values[-1])
    fig.suptitle(
    f"MAXIMUM 700 hPa WIND FORECAST\nValid: {start_t} <--> {end_t}",
    fontsize=14,
    fontweight="bold",
    x=0.5,
    y=0.96,
    ...
    ax.set_position([0.08, 0.13, 0.84, 0.76])
    add_weatherex_logo(fig)
    ...
    times = getvar(ncfile, "Times", timeidx=None)
    def update(frame):
        ax.clear()
        apply_map_features(ax)
        ...
        t_str = format_ist_time(times.values[frame])
        fig.suptitle( f"700 hPa WIND EVOLUTION\nValid: {t_str}",fontsize=15,fontweight="bold", x=0.5,y=0.96,ha="center")
